...
import logging
from data import dubbing_dataset
...
class Trainer:
        ...
        while flag:
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                for j in range(self.args.gradient_accumulation_steps):
                    ...
                    # batch["name"]=np.array(batch["name"])
    ...
    def _setup_dataloader(self):
        train_dataset, val_dataset = dubbing_dataset.dataset(self.args, 'train'), dubbing_dataset.dataset(self.args, 'validation')

        if self.args.dynamic_batching:
            train_sampler = DistributedDynamicBatchSampler(train_dataset, self.args, num_replicas=self.world_size,
                                                           rank=self.rank, shuffle=True, seed=self.args.seed,
                                                           drop_last=True, lengths_list=train_dataset.lengths_list,
                                                           verbose=True, epoch=0)
            valid_sampler = DistributedDynamicBatchSampler(val_dataset, self.args, num_replicas=self.world_size,
                                                           rank=self.rank, shuffle=True, seed=self.args.seed,
                                                           drop_last=True, lengths_list=val_dataset.lengths_list,
                                                           verbose=True, epoch=0)
        else:
            train_sampler = StatefulDistributedSampler(train_dataset, self.args.batch_size // self.world_size,
    ...
    def _setup_models(self):
        model = voicecraft_dub.VoiceCraft_Dub(self.args)
        ...
        if self.args.optimizer_name == "ScaledAdam":
            ...
            init_train = ["adapting_layer", "video_positional_embedding", "fusion_layer"]

            param1 = [p for n, p in model.named_parameters() if
                      any(nd in n for nd in init_train) and p.requires_grad]  # initial training
            param2 = [p for n, p in model.named_parameters() if
                      not any(nd in n for nd in no_decay) and p.requires_grad and not any(
                          nd in n for nd in init_train)]  # w weight decay
            param3 = [p for n, p in model.named_parameters() if
                      any(nd in n for nd in no_decay) and p.requires_grad and not any(
                          nd in n for nd in init_train)]  # w/o weight decay
    ...
    def _setup_optimizer(self):
        if self.args.optimizer_name == "ScaledAdam":
            ...
            learning_rates = [self.args.lr * 100, self.args.lr, self.args.lr]
            param_groups = [{'params': group["params"], "wieght_decay": group["weight_decay"], 'lr': lr} for group, lr
                            in zip(self.trainables, learning_rates)]
            ...
            def lr_lambda(current_step: int):
                ...
                return max(0.0, float(self.total_step - current_step) / float(max(1, self.total_step - warmup_steps)))
